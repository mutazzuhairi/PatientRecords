...
import {SweetAlertPopup} from "./common"
  ...
  static async _request(restRequest, config) {
    ...
    try {
      ...
      if (error.response) {
        ...
        var errorType = error.response?.data?.ErrorType || error.response?.data?.errorType;
        var errorMessage = error.response?.data?.ErrorMessage || error.response?.data?.errorMessage;
         var erorrStatus = error.response?.status;
         if(errorType || errorMessage){
          SweetAlertPopup(errorType,errorMessage);
         }
         else if(erorrStatus==401){
          SweetAlertPopup("Invalid Authentication","you are not authorized!");
          // localStorage.clear();
